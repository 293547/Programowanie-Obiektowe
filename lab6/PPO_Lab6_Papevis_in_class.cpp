...
public:
    ...
    string serialize() const override { 
        return getType() + ";" + getStudentId() + ";" + firstName + ";" + lastName; 
    }
};
...
public:
    ...
    string getType() const override { return "Pracownik"; }
...
public:
    ...
    void saveSinglePerson() {
        ...
        if (foundIndex != -1) {
            ...
            ISerializable* serializableObj = dynamic_cast<ISerializable*>(tabPeople[foundIndex]);
            if (serializableObj) {
                file << serializableObj->serialize() << "\n";
                cout << "Osoba zostala dopisana do pliku.\n";
            }
            file.close();
        } else {
    ...
    void saveWholeList() {
        ...
        for (int i = 0; i < N; i++) {
            if (tabPeople[i] != nullptr) {
                ISerializable* serializableObj = dynamic_cast<ISerializable*>(tabPeople[i]);
                if (serializableObj) {
                    file << serializableObj->serialize() << "\n";
                    count++;
                }
        ...
        file.close();
        cout << "Dopisano " << count << " osob do pliku.\n";
    }
    ...
    void saveSelectedGroup() {
        ...
        else if (groupChoice == 2) targetType = "Pracownik";
        ...
        int count = 0;
        for (int i = 0; i < N; i++) {
            if (tabPeople[i] != nullptr && tabPeople[i]->getType() == targetType) {
                ISerializable* serializableObj = dynamic_cast<ISerializable*>(tabPeople[i]);
                if (serializableObj) {
                    file << serializableObj->serialize() << "\n";
                    count++;
        ...
        file.close();
        cout << "Dopisano " << count << " osob z wybranej grupy do pliku.\n";
    }
};
...
int main() {
    ...
    do {
        ...
        switch (choice) {
            ...
            case 6: registerSystem.saveSinglePerson(); break;
            case 7: registerSystem.saveWholeList(); break;
            case 8: registerSystem.saveSelectedGroup(); break;
